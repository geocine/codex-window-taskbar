...
/// Create a rounded-rectangle tray icon badge showing the remaining percentage.
...
pub fn create_icon(percent: Option<f64>) -> HICON {
    ...
    let fill = Color::from_hex("#3BAFDA");
...
/// Return true when the supplied screen point is inside this app's tray icon.
pub fn contains_point(hwnd: HWND, pt: POINT) -> bool {
    unsafe {
        ...
        match Shell_NotifyIconGetRect(&identifier) {
            Ok(rect) => {
